...
int main() {
    ...
    do {
        try {
            ...
            // Add a permanent message about the active rules list
            string rulesMessage = "Full list of active rules is at the bottom of the report.";

            // Open a file dialog to browse for the .properties file
            char propertiesFileName[MAX_PATH] = "";
            OPENFILENAMEA propertiesOfn;
            ZeroMemory(&propertiesOfn, sizeof(propertiesOfn));
            propertiesOfn.lStructSize = sizeof(propertiesOfn);
            propertiesOfn.hwndOwner = nullptr;
            propertiesOfn.lpstrFilter = "Properties Files (*.properties)\0*.properties\0All Files (*.*)\0*.*\0";
            propertiesOfn.lpstrFile = propertiesFileName;
            propertiesOfn.nMaxFile = MAX_PATH;
            propertiesOfn.Flags = OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST;
            propertiesOfn.lpstrTitle = "Select Rules Properties File";

            if (!GetOpenFileNameA(&propertiesOfn)) {
                cerr << "No file selected or dialog cancelled." << endl;
                continue;
            }
            ...
            html << "<div class='header'><strong>Device Name:</strong> " << deviceName << "<br>"
                ...
                << "<strong>Rules Set Name:</strong> " << rulesSetName << " - " << rulesMessage << "<br></div>";
            ...
            // Add the list of active rules at the bottom of the report
            html << "<h2>Active Rules:</h2><ul>";
