...
// Structure to hold project information from Locations
struct ProjectInfo {
    string project;       // Project name
    string path;          // Project path
    string fsPath;        // Full file system path
    string loc;           // Location
};
...
int main() {
    ...
    // Vector to store project information
    ...
    // Extract project information from Locations section
    XMLElement* locationsElement = root->FirstChildElement("Locations");
    if (locationsElement) {
        cout << "Found Locations section" << endl;
        for (XMLElement* locElement = locationsElement->FirstChildElement("Loc"); locElement; locElement = locElement->NextSiblingElement("Loc")) {
            ProjectInfo info;
            info.project = locElement->Attribute("project") ? locElement->Attribute("project") : "";
            info.path = locElement->Attribute("projPath") ? locElement->Attribute("projPath") : "";
            info.fsPath = locElement->Attribute("fsPath") ? locElement->Attribute("fsPath") : "";
            info.loc = locElement->Attribute("loc") ? locElement->Attribute("loc") : "";

            if (!info.project.empty() || !info.path.empty() || !info.fsPath.empty()) {
                projectInfos.push_back(info);
                cout << "Found project: " << info.project << ", path: " << info.fsPath << endl;
            }
        }
    }
    else {
        cout << "No <Locations> section found." << endl;
    }
    ...
    htmlFile << "<!DOCTYPE html>\n"
        ...
        << "    body { font-family: Arial, sans-serif; margin: 20px; }\n"
        << "    h1 { color: #2c3e50; }\n"
        << "    .project-info { margin-bottom: 15px; color: #34495e; }\n"
        << "    .project-name { font-weight: bold; }\n"
        << "    .project-path { font-family: monospace; background-color: #f8f9fa; padding: 3px 6px; border-radius: 3px; }\n"
        << "    .summary { margin-bottom: 20px; background-color: #f8f9fa; padding: 10px; border-left: 4px solid #2c3e50; }\n"
        << "    .tree { margin-left: 20px; }\n"
        << "    .severity { color: white; font-weight: bold; padding: 10px; margin: 10px 0; border-radius: 5px; }\n"
        << "    .severity-0 { background-color: #3498db; }\n"
    ...
    if (!projectInfos.empty()) {
        htmlFile << "  <div class='project-info'>\n";
        for (const auto& info : projectInfos) {
            htmlFile << "    <p><span class='project-name'>Project:</span> " << info.project << "</p>\n";

            if (!info.fsPath.empty()) {
                htmlFile << "    <p><span class='project-name'>File Path:</span> <span class='project-path'>"
                    << info.fsPath << "</span></p>\n";
            }

            if (!info.path.empty()) {
                htmlFile << "    <p><span class='project-name'>Project Path:</span> <span class='project-path'>"
                    << info.path << "</span></p>\n";
            }
        }
        htmlFile << "  </div>\n";
